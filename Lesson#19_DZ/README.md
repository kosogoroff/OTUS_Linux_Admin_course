...
Ответы а вопросы:
1. Различие между образами и контейнерами Docker: Контейнер - это запущенный экземпляр образа. Из одного образа можно создать несколько контейнеров.
2. Я считаю, что в контейнере Docker кастомное ядро собрать нельзя, так как контейнеры используют ядро хоста.
